...
function drawCard(c,{hero,logo,score,cakes,rank,card,index=0}){
 const paper='#f9f4ee',green=card?.color||'#3d6e5a',mocha='#8a7660';
 ...
 c.fillStyle=mocha;c.font='34px "Songti TC", serif';c.fillText(card?`${String(index+1).padStart(2,'0')} / 06 · ${card.name}`:'把美好，帶回團圓。',540,337);
 ...
 c.fillStyle=green;c.font='40px "Songti TC", serif';c.fillText(card?.line1||'平日，陪你照顧每一份託付。',540,1164);
 c.fillText(card?.line2||'中秋，願你安心享受每一刻團圓。',540,1226);
